...
static void initTunSocket2(String* socket,
                          struct Context* ctx,
                          uint8_t addressPrefix,
                          struct Except* eh)
{
    Log_debug(ctx->logger, "Initializing socket: %s;", socket->bytes);

    struct Iface* tunSocket = TUNSocketInterface_new(
        socket->bytes, ctx->base, ctx->logger, NULL, ctx->alloc);

    Iface_plumb(tunSocket, &ctx->nc->tunAdapt->tunIf);
}
...
static void initTunSocket(Dict* args, void* vcontext, String* txid, struct Allocator* requestAlloc)
{
    struct Context* const ctx = Identity_check((struct Context*) vcontext);

    struct Jmp jmp;
    Jmp_try(jmp) {
        String* socket = Dict_getStringC(args, "socket");
        initTunSocket2(socket, ctx, AddressCalc_ADDRESS_PREFIX_BITS, &jmp.handler);
    } Jmp_catch {
               ...
               bool noSec)
{
    ...
    Admin_registerFunction("Core_initTunSocket", initTunSocket, ctx, true,
        ((struct Admin_FunctionArg[]) {
            { .name = "socket", .required = 1, .type = "String" }
        }), admin);
}
